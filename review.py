...
import os
key_file_path = r"C:/Users/ADMIN/Innomatics/genai/keys/gkey.txt"

try:
    with open(key_file_path, "r") as f:
        key = f.read().strip()  # Ensure to remove any trailing whitespace or newline
    ai.configure(api_key=key)
except FileNotFoundError:
    st.error(f"File not found: {key_file_path}. Please check the file path.")
    st.stop()  # Stop further execution
except Exception as e:
    st.error(f"An unexpected error occurred while reading the API key: {e}")
    st.stop()  # Stop further execution
